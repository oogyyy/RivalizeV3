...
RETURNS BOOLEAN LANGUAGE sql SECURITY DEFINER SET search_path = public STABLE AS $$
  SELECT EXISTS (SELECT 1 FROM team_members WHERE team_id = target_team_id AND user_id = auth.uid() AND role IN ('owner', 'admin'));
$$;

CREATE OR REPLACE FUNCTION public.is_team_creator(target_team_id UUID)
RETURNS BOOLEAN LANGUAGE sql SECURITY DEFINER SET search_path = public STABLE AS $$
  SELECT EXISTS (SELECT 1 FROM teams WHERE id = target_team_id AND created_by = auth.uid());
$$;
...
-- team_members
CREATE POLICY "Team members can view membership" ON team_members FOR SELECT USING (auth.uid() = user_id OR is_team_member(team_id));
CREATE POLICY "Team creators can add themselves as owner" ON team_members FOR INSERT WITH CHECK (auth.uid() = user_id AND role = 'owner' AND is_team_creator(team_id));
CREATE POLICY "Owners and admins can add members" ON team_members FOR INSERT WITH CHECK (is_team_admin(team_id) AND role IN ('admin', 'member'));
CREATE POLICY "Owners and admins can update members" ON team_members FOR UPDATE USING (is_team_admin(team_id)) WITH CHECK (is_team_admin(team_id));
CREATE POLICY "Owners and admins can remove members" ON team_members FOR DELETE USING (is_team_admin(team_id));
