...
export default function HeatmapCanvas({
  ...
  useEffect(() => {
    ...
    if (bgImage) {
      ...
      // Dark scrim so dots stay readable
      ctx.globalAlpha = 0.50
      ctx.fillStyle = '#080c18'
      ctx.fillRect(0, 0, width, height)
      ctx.globalAlpha = 1
    } else {
      // Subtle grid fallback when no map image
      ctx.strokeStyle = 'rgba(255,255,255,0.04)'
      ctx.lineWidth = 1
      const g = 64
      for (let x = 0; x <= width;  x += g) { ctx.beginPath(); ctx.moveTo(x, 0);  ctx.lineTo(x, height); ctx.stroke() }
      for (let y = 0; y <= height; y += g) { ctx.beginPath(); ctx.moveTo(0, y);  ctx.lineTo(width, y);  ctx.stroke() }
    ...
    ctx.fillText(`${filtered.length} events`, width - 82, height - 10)

  }, [filtered, bgImage, mapName, width, height])
  ...
  const hasCfg = !!MAP_CONFIGS[mapName]
