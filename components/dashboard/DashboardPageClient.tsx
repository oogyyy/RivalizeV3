...
import { ArrowRight, Award, BarChart2, Brain, Film, Star, TrendingUp, Zap, Users } from 'lucide-react'
...
export default function DashboardPageClient({
  ...
  const [activeTab, setActiveTab] = useState<'overview' | 'performance' | 'community' | 'ai-brief'>('overview')
  ...
  return (
    <div style={{ flex: 1, overflowY: 'auto', padding: '24px 28px', display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      {/* ── Performance Tab: Recent Demos + Map Performance ── */}
      ...
      {activeTab === 'community' && (
        communityFeed.length > 0 ? (
          ...
          <div style={{ padding: '48px 16px', textAlign: 'center', borderRadius: 14, border: '1px solid var(--border)', background: 'var(--card)' }}>
            <Users size={28} style={{ color: 'var(--faint)', margin: '0 auto 12px' }} />
            <p style={{ fontSize: 14, fontWeight: 500, color: 'var(--text)', marginBottom: 6 }}>No community reports yet</p>
            <p style={{ fontSize: 12, color: 'var(--muted)' }}>Public scout reports from the community will appear here.</p>
          </div>
        )
      )}
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            ...
            <Link href="/ai-coach" style={{ textDecoration: 'none' }}>
              <div style={{ padding: '9px 16px', borderRadius: 8, background: '#7b5cf5', cursor: 'pointer' }}>
